...
| 15I | Payments & Receipts Platform (`payments.html` + `js/paymentData.js`, the platform's fifth automatic-posting consumer and its first write beyond invoice-time posting — closes the gap 15E–15H all inherited silently, where `create_sale()`/`create_purchase()` could only record payment at invoice time and a receivable or payable, once created, could never be reduced. One new RPC, `record_payment()`, atomically settles exactly one open document per call and writes no journal entry itself; posting stays with `post_journal_entry()`, invoked through `AccountingPlatform.post()` as a second, best-effort step, the same two-step contract 15B established. Two new posting providers, `receiptPostingProvider.js`/`paymentPostingProvider.js`, registered for `VOUCHER_TYPES.RECEIPT`/`PAYMENT` — both declared since 15A and unused until now — resolving only `cashAccount` plus `accountsReceivableAccount`/`accountsPayableAccount`, no new account role. Governed by ADR-0016: every settlement posts to Cash regardless of recorded `payment_type` (no bank-account/mode-mapping infrastructure exists); allocation is document-level via `payments.invoice_id` for a receipt, or a direct `purchases` update for a payment, since `payments` has no `purchase_id` column and never has — a disclosed, pre-existing limitation, not a regression; payments are immutable in this milestone, the same posture every other document in the application already has. **Zero new schema object beyond the one RPC**, and zero change to any earlier posting provider or screen. Dedicated offline test file, `js/paymentData.test.html` (42 checks). Live-validated in two passes against a real Supabase staging project — direct RPC/SQL, then the actual rendered screen clicked through end to end, both directions, both partial and full settlement, all five downstream statements. Tag `payments-receipts-platform-v1.0`) |

## 4. Current Repository Status

| | |
|---|---|
| **Current Branch** | `master` (15I merged via PR #19, merge commit `761c061`, and tagged `payments-receipts-platform-v1.0`; its feature branch `milestone-15i-payments-receipts` is retired at the close of the same release, as 15H's was. 15H before it merged via PR #17, merge commit `3dd117a`, and its own feature branch has been deleted) |
| **Latest Release** | `payments-receipts-platform-v1.0` (15I's own tag, pointing at merge commit `761c061`; 15H's `balance-sheet-platform-v1.0` points at `3dd117a`. 15C and the Purchase Posting hotfix remain merged but not separately tagged, per the same "each milestone's PR documents its own completion, the next milestone's docs update adds the confirmed checkpoint row" convention this table's §8 follows) |
...
**Milestone 15I (Payments & Receipts Platform) is released.** It merged via PR #19 (merge
commit `761c061`) and is tagged `payments-receipts-platform-v1.0`, so it has moved out of
this section into §3's Completed Milestones table and §8's Repository Checkpoints, the same
way 15H moved out on merging. Its architecture is recorded in ADR-0016 and in
...
**Carried forward as a named Accounting Foundation gap, deliberately not solved in 15H or
15I:**
...
- `accounting-platform-architecture.md` — the Accounting Platform's permanent architecture
  ...
  change and, unlike 15F and 15G, with no change to any earlier consumer either; Milestone
  15I added Payments & Receipts (`payments.html`, `js/paymentData.js`, ADR-0016) — the
  platform's fifth automatic-posting consumer and its first write beyond invoice-time
  posting, via the one new `record_payment()` RPC, with zero change to any earlier posting
  provider or screen. See `docs/releases/accounting-platform-foundation-v1.0.md` (15A) —
  15B/15C/15D/15E/15F/15G/15H/15I have no separate release checkpoint documents, only their
  tags (`journal-engine-v1.0`, `journal-inquiry-platform-v1.0`, `general-ledger-platform-v1.0`,
  `trial-balance-platform-v1.0`, `profit-loss-platform-v1.0`,
  `balance-sheet-platform-v1.0`, `payments-receipts-platform-v1.0`; 15C is untagged, folded
  into `journal-inquiry-platform-v1.0`'s history).
...
| `payments-receipts-platform-v1.0` | Completion of Milestone 15I — the Payments & Receipts Platform: `payments.html` + `js/paymentData.js`, letting a customer receipt or supplier payment be recorded against an already-open invoice/purchase for the first time, closing the gap 15E–15H all inherited from `create_sale()`/`create_purchase()` only ever recording payment at invoice time. One new RPC, `record_payment()`, atomically settles exactly one open document per call (`payments` row, the document's own `amount_paid`/`amount_due`, and the party's `current_balance`, all in one transaction) and writes no journal entry itself — posting stays with `post_journal_entry()`, invoked through `AccountingPlatform.post()` as a second, best-effort step. Two new posting providers, `receiptPostingProvider.js`/`paymentPostingProvider.js`, resolve only `cashAccount` plus `accountsReceivableAccount`/`accountsPayableAccount` — no new account role. Governed by ADR-0016, which records a disclosed, pre-existing limitation found during the pre-code audit rather than introduced by this milestone: `payments` has no `purchase_id` column and never has, so a supplier settlement is not traceable back to its purchase from the `payments` table alone. **Zero new schema object beyond the one RPC**, and zero change to any earlier posting provider, screen, or RLS policy. Live-validated in two passes against a real Supabase staging project: direct RPC/SQL calls, then the actual rendered screen served from the working tree and clicked through end to end — both settlement directions, both partial and full, amount validation, already-settled-document exclusion, cross-company rejection (verified against both the raw RPC and the real `recordSettlement()` function), and all five downstream statements (Journal Register, General Ledger, Trial Balance, Profit & Loss, Balance Sheet) opened for real in two companies. Live UI testing found and fixed one real defect (the party-search field not clearing between dialog opens), reverified after the fix. Dedicated offline test file (`js/paymentData.test.html`, 42 checks). Regression 1867/1867 across 28 files. Merge commit `761c061`. |
